...
import com.cefisi.registration.user.OnRegistrationCompleteEvent;
...
//    
    ...
    public String showRegistrationForm(final HttpServletRequest request, final Model model) {
        ...
        @RequestMapping(value = "/reg-new-user", method = RequestMethod.POST)
    public ModelAndView registerUserAccount(@ModelAttribute("user") @Valid final Personne userDto,
             final HttpServletRequest request, final Errors errors) {
        //LOGGER.debug("Registering user account with information: {}", userDto);

        final Personne registered = userService.registerNewUserAccount(userDto);
        if (registered == null) {
